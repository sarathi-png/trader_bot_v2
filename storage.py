...
class Store:
    ...
    def close_position(self, position_id, exit_price, pnl, reason):
        with self._tx() as c:
            c.execute("UPDATE positions SET status='CLOSED',exit_price=?,exit_reason=?,pnl=?,closed_at=? WHERE id=?",(exit_price,reason,pnl,self._now(),position_id)); c.execute("INSERT INTO daily_risk(day,realized_pnl) VALUES(date('now'),?) ON CONFLICT(day) DO UPDATE SET realized_pnl=realized_pnl+excluded.realized_pnl",(pnl,))
        with self._tx() as c2:
            r=c2.execute('SELECT * FROM positions WHERE id=?',(position_id,)).fetchone()
    ...
    def health(self):
        with self._tx() as c:
            r=c.execute('SELECT * FROM analysis_runs ORDER BY started_at DESC LIMIT 1').fetchone(); return {'last_run':dict(r) if r else None,'signal_count':c.execute('SELECT COUNT(*) FROM signals').fetchone()[0],'open_positions':c.execute("SELECT COUNT(*) FROM positions WHERE status='OPEN'").fetchone()[0],'daily_pnl':self.daily_realized_pnl(),'db':self.path}
_GLOBAL_STORE=None
